...
IS vs OUGHT note - Any referenced repo path is OUGHT unless verified by evidence (repo tree listing / commit / CI check). If OUGHT, add a bootstrap dependency that creates it.

## GAP-00 Repo skeleton (paths)

Work item BR-00.1

Task Create repository skeleton folders for specs, schemas, fixtures, tools, src, and tests.
...
Task Define the cid: minting and validation spec.
...
Task Implement cid: library and tests.
...
Task Define ledger API contract (append, read, as-of, replay) and ordering rules.
...
Task Implement append-only ledger service with single-writer ordering authority.
...
Task Define IssuerProof format and verification algorithm (MVP-single mechanism).
...
Task Implement IssuerProof verifier and negative tests.
...
Task Define evaluator boundary interface and commit semantics.
...
Task Implement evaluator stub and commit-boundary enforcement tests.
...
Task Define release manifest and load-manifest schemas + fixtures.
...
Task Implement packager that emits manifest + hashes + file list.
...
Task Implement verifier + loader rejection logic with tamper tests.
...
Task Create minimal SupportOntologyRelease payload + first package.
...
Task Create minimal SCR_TBox_Release payload + first tenant package.
